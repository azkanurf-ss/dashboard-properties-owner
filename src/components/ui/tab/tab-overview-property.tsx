...
const TabOverviewProperty = ({ data }: { data: PropertyModel }) => {
  const readableTypes = data?.type && data?.type?.map((data) => convertText(data))
  const readableStyles = data?.style && data?.style?.map((data) => convertText(data))
  ...
  return (
    <TabContentContainer>
      <div className="el-grid">
        ...
        <div className="el-col-split">
          <Subtitle hasNoMargin>Council Tax: {data?.councilTax || 'No Data'}</Subtitle>
          <Subtitle hasNoMargin className="el-mt4">
