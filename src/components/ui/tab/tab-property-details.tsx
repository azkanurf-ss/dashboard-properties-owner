...
import { MdOutlineMeetingRoom, MdOutlineBathroom, MdOutlineYard } from 'react-icons/md'
...
const TabPropertyDetails = ({ data }: { data: PropertyModel }) => {
  const readableParking = data?.parking && data?.parking?.map((data) => convertText(data))
  const readableSituation = data?.situation && data?.situation?.map((data) => convertText(data))
  ...
  const handleCollapseClosed = (index: number) => {
    const newOpenedRoomsDetails = openedRoomsDetails.filter((data) => data !== index)
        ...
        <div className="el-col-split">
          ...
          <div className="el-flex el-flex-row">
            <Subtitle>
              ...
              Assets:
              {readableSituation && readableSituation?.length === 0 && ' No Data'}
            </Subtitle>
            {readableSituation && readableSituation?.length > 0 && (
              <div className="el-pt3 el-pl2">
                {readableSituation?.map((data, index) => (
                  <div key={index} style={{ display: 'block' }} className="el-flex">
                    <BiCheck />
                    <span>{data}</span>
                  </div>
                ))}
        ...
        <div className="el-col-split">
          ...
          <Subtitle>
            ...
            {data?.internalArea?.min
              ? `${numberWithCommas(data?.internalArea?.min)} ${convertText(data?.internalArea?.type)}`
              : 'No Data'}
          </Subtitle>
          <Subtitle>
            ...
            {data?.externalArea?.min
              ? `${numberWithCommas(data?.externalArea?.min)} ${data?.externalArea?.type}`
              : 'No Data'}
          </Subtitle>
        </div>
      </div>
          ...
          data?.rooms.map((room, index) => (
            <div key={index}>
              <Collapsible
                ...
                onOpen={() => setOpenedRoomsDetails(openedRoomsDetails.concat(index))}
